...
def agedist(data):
    ...
    # Skewed feature plotting
    for i, feature in enumerate(['Age']):
        ax = fig.add_subplot(1, 2, i+1)
        ax.hist(data[feature], bins = 25, color = '#00A0A0')
        ax.set_title("'%s' Feature Distribution"%(feature), fontsize = 14)
        ax.set_xlabel('Age')
        ...
        ax.set_yticklabels([0, 5000, 10000, 15000, 20000, '>20000'])

    # Plot aesthetics
    fig.suptitle("Age Distribution", fontsize = 16, y = 1.03)
...
def evaluate(results, accuracy, f1):
    ...
    fig, ax = pl.subplots(2,3, figsize = (50,45))
...
def wealthgroup(data):
	...
	values = [(data['WealthGroup'] == columns[0]).sum(), (data['WealthGroup'] == columns[1]).sum(), (data['WealthGroup'] == columns[2]).sum(),(data['WealthGroup'] == columns[3]).sum()]
